...
import { BoxplotGenerator } from '../BoxplotGenerator.js';

describe('BoxplotGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(BoxplotGenerator.metadata);
      assert.ok(BoxplotGenerator.metadata.id);
      assert.ok(BoxplotGenerator.metadata.name);
      assert.ok(BoxplotGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(BoxplotGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(BoxplotGenerator.schema);
      assert.ok(Array.isArray(BoxplotGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(BoxplotGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      BoxplotGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      const config = {};
      BoxplotGenerator.schema.fields
        .filter(f => f.required)
        .forEach(f => {
          if (f.type === 'field') {
      ...
      const generator = new BoxplotGenerator(config);
      const data = BoxplotGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = BoxplotGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for BoxplotGenerator');
        return;
      }
      ...
      const generator = new BoxplotGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...BoxplotGenerator.example?.config };
      assert.doesNotThrow(() => {
        new BoxplotGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = BoxplotGenerator.schema.fields.filter(f => f.required);
      const config = {};
