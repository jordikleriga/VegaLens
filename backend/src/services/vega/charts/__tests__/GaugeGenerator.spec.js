...
import { GaugeGenerator } from '../GaugeGenerator.js';

describe('GaugeGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(GaugeGenerator.metadata);
      assert.ok(GaugeGenerator.metadata.id);
      assert.ok(GaugeGenerator.metadata.name);
      assert.ok(GaugeGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(GaugeGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(GaugeGenerator.schema);
      assert.ok(Array.isArray(GaugeGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(GaugeGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      GaugeGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      ...
      const generator = new GaugeGenerator(config);
      const data = GaugeGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = GaugeGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for GaugeGenerator');
        return;
      }
      ...
      const generator = new GaugeGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...GaugeGenerator.example?.config };
      assert.doesNotThrow(() => {
        new GaugeGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = GaugeGenerator.schema.fields.filter(f => f.required);
      const config = {};
