...
import { MetricGenerator } from '../MetricGenerator.js';

describe('MetricGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(MetricGenerator.metadata);
      assert.ok(MetricGenerator.metadata.id);
      assert.ok(MetricGenerator.metadata.name);
      assert.ok(MetricGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(MetricGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(MetricGenerator.schema);
      assert.ok(Array.isArray(MetricGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(MetricGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      MetricGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      ...
      const generator = new MetricGenerator(config);
      const data = MetricGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = MetricGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for MetricGenerator');
        return;
      }
      ...
      const generator = new MetricGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...MetricGenerator.example?.config };
      assert.doesNotThrow(() => {
        new MetricGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = MetricGenerator.schema.fields.filter(f => f.required);
      const config = {};
