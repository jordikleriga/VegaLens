...
import { RadialGenerator } from '../RadialGenerator.js';

describe('RadialGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(RadialGenerator.metadata);
      assert.ok(RadialGenerator.metadata.id);
      assert.ok(RadialGenerator.metadata.name);
      assert.ok(RadialGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(RadialGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(RadialGenerator.schema);
      assert.ok(Array.isArray(RadialGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(RadialGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      RadialGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      ...
      const generator = new RadialGenerator(config);
      const data = RadialGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = RadialGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for RadialGenerator');
        return;
      }
      ...
      const generator = new RadialGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...RadialGenerator.example?.config };
      assert.doesNotThrow(() => {
        new RadialGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = RadialGenerator.schema.fields.filter(f => f.required);
      const config = {};
