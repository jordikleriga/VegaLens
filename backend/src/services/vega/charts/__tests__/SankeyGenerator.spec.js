...
import { SankeyGenerator } from '../SankeyGenerator.js';

describe('SankeyGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(SankeyGenerator.metadata);
      assert.ok(SankeyGenerator.metadata.id);
      assert.ok(SankeyGenerator.metadata.name);
      assert.ok(SankeyGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(SankeyGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(SankeyGenerator.schema);
      assert.ok(Array.isArray(SankeyGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(SankeyGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      SankeyGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      ...
      const generator = new SankeyGenerator(config);
      const data = SankeyGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = SankeyGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for SankeyGenerator');
        return;
      }
      ...
      const generator = new SankeyGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...SankeyGenerator.example?.config };
      assert.doesNotThrow(() => {
        new SankeyGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = SankeyGenerator.schema.fields.filter(f => f.required);
      const config = {};
