...
import { RadarGenerator } from '../RadarGenerator.js';

describe('RadarGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(RadarGenerator.metadata);
      assert.ok(RadarGenerator.metadata.id);
      assert.ok(RadarGenerator.metadata.name);
      assert.ok(RadarGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(RadarGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(RadarGenerator.schema);
      assert.ok(Array.isArray(RadarGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(RadarGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      RadarGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      ...
      const data = RadarGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = RadarGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for RadarGenerator');
        return;
      }
      ...
      const generator = new RadarGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...RadarGenerator.example?.config };
      assert.doesNotThrow(() => {
        new RadarGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = RadarGenerator.schema.fields.filter(f => f.required);
      const config = {};
