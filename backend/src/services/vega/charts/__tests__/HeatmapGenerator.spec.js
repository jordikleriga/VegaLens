...
describe('HeatmapGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(HeatmapGenerator.metadata);
      assert.ok(HeatmapGenerator.metadata.id);
      assert.ok(HeatmapGenerator.metadata.name);
      assert.ok(HeatmapGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(HeatmapGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(HeatmapGenerator.schema);
      assert.ok(Array.isArray(HeatmapGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(HeatmapGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      HeatmapGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      const config = {};
      HeatmapGenerator.schema.fields
        .filter(f => f.required)
        .forEach(f => {
          if (f.type === 'field') {
      ...
      const data = HeatmapGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = HeatmapGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for HeatmapGenerator');
        return;
      }
      ...
      const generator = new HeatmapGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...HeatmapGenerator.example?.config };
      assert.doesNotThrow(() => {
        new HeatmapGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = HeatmapGenerator.schema.fields.filter(f => f.required);
      const config = {};
