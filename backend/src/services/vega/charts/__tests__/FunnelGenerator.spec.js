...
import { FunnelGenerator } from '../FunnelGenerator.js';

describe('FunnelGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(FunnelGenerator.metadata);
      assert.ok(FunnelGenerator.metadata.id);
      assert.ok(FunnelGenerator.metadata.name);
      assert.ok(FunnelGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(FunnelGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(FunnelGenerator.schema);
      assert.ok(Array.isArray(FunnelGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(FunnelGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      FunnelGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      ...
      const generator = new FunnelGenerator(config);
      const data = FunnelGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = FunnelGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for FunnelGenerator');
        return;
      }
      ...
      const generator = new FunnelGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...FunnelGenerator.example?.config };
      assert.doesNotThrow(() => {
        new FunnelGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = FunnelGenerator.schema.fields.filter(f => f.required);
      const config = {};
