...
describe('WordcloudGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(WordcloudGenerator.metadata);
      assert.ok(WordcloudGenerator.metadata.id);
      assert.ok(WordcloudGenerator.metadata.name);
      assert.ok(WordcloudGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(WordcloudGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(WordcloudGenerator.schema);
      assert.ok(Array.isArray(WordcloudGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(WordcloudGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      WordcloudGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      const config = {};
      WordcloudGenerator.schema.fields
        .filter(f => f.required)
        .forEach(f => {
          if (f.type === 'field') {
      ...
      const generator = new WordcloudGenerator(config);
      const data = WordcloudGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = WordcloudGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for WordcloudGenerator');
        return;
      }
      ...
      const generator = new WordcloudGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...WordcloudGenerator.example?.config };
      assert.doesNotThrow(() => {
        new WordcloudGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = WordcloudGenerator.schema.fields.filter(f => f.required);
      const config = {};
