...
import { WaterfallGenerator } from '../WaterfallGenerator.js';

describe('WaterfallGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(WaterfallGenerator.metadata);
      assert.ok(WaterfallGenerator.metadata.id);
      assert.ok(WaterfallGenerator.metadata.name);
      assert.ok(WaterfallGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(WaterfallGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(WaterfallGenerator.schema);
      assert.ok(Array.isArray(WaterfallGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(WaterfallGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      WaterfallGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      const config = {};
      WaterfallGenerator.schema.fields
        .filter(f => f.required)
        .forEach(f => {
          if (f.type === 'field') {
      ...
      const generator = new WaterfallGenerator(config);
      const data = WaterfallGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = WaterfallGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for WaterfallGenerator');
        return;
      }
      ...
      const generator = new WaterfallGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...WaterfallGenerator.example?.config };
      assert.doesNotThrow(() => {
        new WaterfallGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = WaterfallGenerator.schema.fields.filter(f => f.required);
      const config = {};
