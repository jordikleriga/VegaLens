...
describe('CometGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(CometGenerator.metadata);
      assert.ok(CometGenerator.metadata.id);
      assert.ok(CometGenerator.metadata.name);
      assert.ok(CometGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(CometGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(CometGenerator.schema);
      assert.ok(Array.isArray(CometGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(CometGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      CometGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      ...
      const generator = new CometGenerator(config);
      const data = CometGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = CometGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for CometGenerator');
        return;
      }
      ...
      const generator = new CometGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...CometGenerator.example?.config };
      assert.doesNotThrow(() => {
        new CometGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = CometGenerator.schema.fields.filter(f => f.required);
      const config = {};
