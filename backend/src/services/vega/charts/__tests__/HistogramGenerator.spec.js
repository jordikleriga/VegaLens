...
import { HistogramGenerator } from '../HistogramGenerator.js';

describe('HistogramGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(HistogramGenerator.metadata);
      assert.ok(HistogramGenerator.metadata.id);
      assert.ok(HistogramGenerator.metadata.name);
      assert.ok(HistogramGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(HistogramGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(HistogramGenerator.schema);
      assert.ok(Array.isArray(HistogramGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(HistogramGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      HistogramGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      const config = {};
      HistogramGenerator.schema.fields
        .filter(f => f.required)
        .forEach(f => {
          if (f.type === 'field') {
      ...
      const generator = new HistogramGenerator(config);
      const data = HistogramGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = HistogramGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for HistogramGenerator');
        return;
      }
      ...
      const generator = new HistogramGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...HistogramGenerator.example?.config };
      assert.doesNotThrow(() => {
        new HistogramGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = HistogramGenerator.schema.fields.filter(f => f.required);
      const config = {};
