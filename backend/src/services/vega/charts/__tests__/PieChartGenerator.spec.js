...
import { PieChartGenerator } from '../PieChartGenerator.js';

describe('PieChartGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(PieChartGenerator.metadata);
      assert.ok(PieChartGenerator.metadata.id);
      assert.ok(PieChartGenerator.metadata.name);
      assert.ok(PieChartGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(PieChartGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(PieChartGenerator.schema);
      assert.ok(Array.isArray(PieChartGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(PieChartGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      PieChartGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      const config = {};
      PieChartGenerator.schema.fields
        .filter(f => f.required)
        .forEach(f => {
          if (f.type === 'field') {
      ...
      const generator = new PieChartGenerator(config);
      const data = PieChartGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = PieChartGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for PieChartGenerator');
        return;
      }
      ...
      const generator = new PieChartGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...PieChartGenerator.example?.config };
      assert.doesNotThrow(() => {
        new PieChartGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = PieChartGenerator.schema.fields.filter(f => f.required);
      const config = {};
