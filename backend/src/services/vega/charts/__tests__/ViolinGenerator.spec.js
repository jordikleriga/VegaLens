...
import { ViolinGenerator } from '../ViolinGenerator.js';

describe('ViolinGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(ViolinGenerator.metadata);
      assert.ok(ViolinGenerator.metadata.id);
      assert.ok(ViolinGenerator.metadata.name);
      assert.ok(ViolinGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(ViolinGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(ViolinGenerator.schema);
      assert.ok(Array.isArray(ViolinGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(ViolinGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      ViolinGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      ...
      const generator = new ViolinGenerator(config);
      const data = ViolinGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = ViolinGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for ViolinGenerator');
        return;
      }
      ...
      const generator = new ViolinGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...ViolinGenerator.example?.config };
      assert.doesNotThrow(() => {
        new ViolinGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = ViolinGenerator.schema.fields.filter(f => f.required);
      const config = {};
