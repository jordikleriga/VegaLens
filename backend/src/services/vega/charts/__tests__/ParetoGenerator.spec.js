...
import { ParetoGenerator } from '../ParetoGenerator.js';

describe('ParetoGenerator', () => {
  describe('Metadata', () => {
    it('should have valid metadata', () => {
      assert.ok(ParetoGenerator.metadata);
      assert.ok(ParetoGenerator.metadata.id);
      assert.ok(ParetoGenerator.metadata.name);
      assert.ok(ParetoGenerator.metadata.category);
    });

    it('should have correct ID format', () => {
      assert.match(ParetoGenerator.metadata.id, /^[a-z_]+$/);
    });
  });

  describe('Schema Validation', () => {
    it('should have a valid schema', () => {
      assert.ok(ParetoGenerator.schema);
      assert.ok(Array.isArray(ParetoGenerator.schema.fields));
    });

    it('should have fields defined in schema', () => {
      assert.ok(ParetoGenerator.schema.fields.length > 0, 'Should have at least one field');
    });

    it('should have valid field types', () => {
      ParetoGenerator.schema.fields.forEach(field => {
  ...
  describe('Preview Generation', () => {
    it('should generate valid Vega-Lite spec', () => {
      ...
      const generator = new ParetoGenerator(config);
      const data = ParetoGenerator.example?.data || [];
  ...
  describe('Example Data', () => {
    it('should provide a working example', () => {
      const example = ParetoGenerator.example;
      if (!example) {
        console.log('  ⚠ No example provided for ParetoGenerator');
        return;
      }
      ...
      const generator = new ParetoGenerator(example.config);
  ...
  describe('Config Validation', () => {
    it('should accept valid config', () => {
      const config = { ...ParetoGenerator.example?.config };
      assert.doesNotThrow(() => {
        new ParetoGenerator(config);
      });
    });

    it('should handle missing optional fields', () => {
      const requiredFields = ParetoGenerator.schema.fields.filter(f => f.required);
      const config = {};
